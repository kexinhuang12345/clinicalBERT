...
class readmissionProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train_discharge.csv")))
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "train_discharge.csv")), "train")
    
    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "val_discharge.csv")), "val")
    
    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "test_discharge.csv")), "test")
...
def main():
    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                if step >= 4500:
                    if step % 500 == 0:
                        string = './earlystopping_model_discharge/pytorch_model_discharge_bce_at'+str(step)+'.bin'
            ...
            if epo == 0:
                torch.save(model.state_dict(), './pytorch_model_discharge_bce_epo1.bin')
            if epo == 1:
                torch.save(model.state_dict(), './pytorch_model_discharge_bce_epo2.bin')
        fig1 = plt.figure()
    ...
    if args.do_eval:
        ...
        df.to_csv('logits_info_bce_discharge_dev.csv')
        
        tn, fp, fn, tp = confusion_matrix(true_labels, pred_labels).ravel()
        print(classification_report(true_labels, pred_labels, target_names=['Neg Readmit','Pos Readmit']))
        #print(true_labels)
        #print(pred_labels)
        #print(logits_history1)
        #print(len(logits_history1))
        print ('tn:{}fp:{}fn:{}tp:{}'.format(tn,fp,fn,tp))
       
        print (sum(np.array(pred_labels) == np.array(true_labels)) / len(true_labels))
        fpr_keras, tpr_keras, thresholds_keras = roc_curve(true_labels, logits_history)
        ...
        plt.legend(loc='best')
        plt.show()
        fig2.savefig('auc_roc.png', dpi=fig2.dpi)
        ...
        result = {'eval_loss': eval_loss,
                  ...
                  'loss': train_loss/number_training_steps}
